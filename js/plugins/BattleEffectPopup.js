 ...
 *
 * @param 会心一击
 * @desc 会心一击発生時のポップアップメッセージまたはファイル名です。(img/pictures/)拡張子不要
 * @default CRITICAL!
 * @require 1
 * @dir img/pictures/
 * @type file
 *
 * @param 会心一击カラー
 * @desc 会心一击発生時の文字のフラッシュ色です。R(赤),G(緑),B(青),A(強さ)の順番でカンマ(,)区切りで指定。
 * @default 255,0,0,255
 *
 * @param 回避
...
(function() {
    ...
    // パラメータの取得と整形
    //=============================================================================
    var paramCritical         = getParamString(['Critical', '会心一击']);
    var paramCriticalColor    = getParamArrayNumber(['CriticalColor', '会心一击カラー'], 0, 256);
