 ...
 * 戦闘逃走時の処理について
 *-----------------------------------------------------------------------------
 * プラグインパラメータ<Custom Escape Event>を 1 に設定していると
 * 逃走時の処理を独自に設定することができます。
 * 
...
FTKR.EBE.battleEnd = {
    ...
    customE : Number(parameters['Custom Escape Event'] || 0),
    victory : Number(parameters['Victory Event'] || 0),
    defeat  : Number(parameters['Defeat Event'] || 0),
    escape  : Number(parameters['Escape Event'] || 0),
};
        ...
        if (this._checkEbeBattleEvent) {
            switch(this._battleEndPattern) {
                ...
                case 1:
                    ...
                    FTKR.EBE.BattleManager_processAbort.call(this);
                    return true;
                case 2:
...
FTKR.EBE.BattleManager_processAbort = BattleManager.processAbort;
BattleManager.processAbort = function() {
    if (FTKR.EBE.battleEnd.escape && !this._checkEbeBattleEvent) {
        this._phase = 'turn';
...
BattleManager.isActed = function(battler) {
    return this._actionBattlers && !this._actionBattlers.contains(battler);
};
