...
(function() {
            ...
            if(randCom != null) {
                ...
                randCom.forEach(function(randComParmOrg) {
                    ...
                    for(var i=1;i<randComParm.length;i+=2) {
						...
						var num = 1;
						if($gameActors.actor(1).QTE && $gameActors.actor(1).QTE.avoid){}
						else{num = Math.round(Number(randComParm[i+1]) * (200 - $gameVariables.value(1092)) / 200);}
                        var result = "";
                        for(var j=0;j<num;j++) {
    ...
    HzInputCommand.prototype.checkInput = function() {
        if(Input.isTriggered ('ok')) {
        ...
        } else if($gameVariables.value(4762) < 0) {
			return 0;
            ...
            if(this._command.charAt(this._cursorIdx) === 'x') {
				if($gameActors.actor(1).mp >= $gameActors.actor(1).mmp / 10 && $gameVariables.value(1212) <= 0)	return 3;
                return 1;
            } else {
                return 2;
            }
    ...
    HzInputCommand.prototype.update = function() {
        ...
        if(result === 1) {
			...
			$gameActors.actor(1).QTE.magic = true;
			$gameSwitches.setValue(this._switchNo, true);
            return false;
		}
        return true;
