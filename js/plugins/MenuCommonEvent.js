...
/*     if (param.maxMenuPicture > 0) {
    ...
    Scene_MenuBase.prototype.updateChildren = function() {
        ...
        // }
        var children = this.children;  // 缓存子元素数组
        var hasCommonEvent = this.hasCommonEvent();  // 缓存常用事件检查结果
        var isStopWindow = child => child.update && !this.isStopWindow(child);  // 缓存是否停止窗口的检查函数
    
        if (hasCommonEvent) {
            children.filter(isStopWindow).forEach(child => child.update());
        } else {
            _Scene_MenuBase_updateChildren.apply(this, arguments);
        }
    };
